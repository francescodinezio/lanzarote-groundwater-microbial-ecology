taxrank_heatmap <- function(df,
  ...
  df2 <- df %>%
    ...
    dplyr::filter(!is.na(Group), !is.na(taxon))
  
  if (!show_unclassified) {
    df2 <- df2 %>% dplyr::filter(taxon != "Unclassified")
  }
  
  tax_sample <- df2 %>%
    dplyr::group_by(Sample, Group, taxon) %>%
    dplyr::summarise(rel_abund = sum(rel_abund, na.rm = TRUE), .groups = "drop")
  
  tax_habitat <- tax_sample %>%
    ...
    tidyr::complete(Group, taxon, fill = list(mean_rel_abund = 0))
  
  top_taxa <- tax_habitat %>%
    ...
    dplyr::arrange(dplyr::desc(mean_abund), taxon) %>%
    dplyr::slice_head(n = n_show)
  
  hm <- tax_habitat %>%
    dplyr::filter(taxon %in% top_taxa$taxon) %>%
    dplyr::left_join(top_taxa[, c("taxon", "best_group", "mean_abund")], by = "taxon")
  
  tax_order <- hm %>%
    dplyr::distinct(taxon, best_group, mean_abund) %>%
    dplyr::mutate(best_group = factor(best_group, levels = hab_order)) %>%
    dplyr::arrange(best_group, dplyr::desc(mean_abund), taxon) %>%
    dplyr::pull(taxon)
  ...
  hm <- hm %>%
    dplyr::mutate(
      ...
      label = dplyr::if_else(mean_val < 0.0001, "<0.001", sprintf("%.3f", mean_val)),
  ...
  ggplot2::ggplot(hm, ggplot2::aes(x = Group, y = taxon, fill = mean_val)) +
